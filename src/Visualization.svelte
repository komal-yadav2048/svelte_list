<script>
  ...
  $: xScale = scaleLinear()
    .domain([0, max(data, (d) => d.value) * 1.25])
    .range([0, width])
    .nice();

  // vertically consider one band for each item
  $: yScale = scaleBand().domain(range(data.length)).range([0, height]);
  ...
  const arcGeneratorSlice = arc()
    .padAngle(0.05)
    .innerRadius(width / 3)
    .outerRadius(width / 2);

  const arcGeneratorLabels = arc()
      ...
      >
        <g transform="translate({pieChartMargin} {pieChartMargin})">
          <g transform="translate({width / 2} {height / 2})">
            {#each pieData as d}
              <path d={arcGeneratorSlice(d)} fill={d.data.color} />
              <text
                transform="translate({arcGeneratorLabels.centroid(
                  d
                )[0]} {arcGeneratorLabels.centroid(d)[1]})"
                font-size="18"
                fill="currentColor"
                text-anchor="middle"
                dominant-baseline="middle"
